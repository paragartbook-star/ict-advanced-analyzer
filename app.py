    ...
    def fetch_stock_data(ticker: str) -> Dict:
        try:
            ...
            ema_50 = close_prices.ewm(span=50, adjust=False).mean().iloc[-1] if len(close_prices) >= 50 else \
            close_prices.iloc[-1]
            ...
            return {
                ...
                'price_change_24h': ((close_prices.iloc[-1] - close_prices.iloc[-2]) / close_prices.iloc[-2] * 100) if len(
                    close_prices) > 1 else 0,
                'rsi': rsi,
                'macd': macd,
                'macd_signal': signal,
                'ema_50': ema_50,
    ...
    # ═══════════════════════════════════════════════════════════════════════════════
    # ADVANCED CHARTING WITH ICT OVERLAYS
    # ═══════════════════════════════════════════════════════════════════════════════
    ...
    def display_watchlist():
        ...
        for item in st.session_state.watchlist:
            ...
            with col5:
                if st.button("🗑️", key=f"del_{item['symbol']}"):
    ...
    def display_portfolio():
        ...
        for position in st.session_state.portfolio:
            ...
            with col4:
                ...
                st.markdown(f":{color}[${pnl:.2f} ({pnl_pct:.1f}%)]")
            with col5:
    ...
    def main():
        st.title("📊 ICT Advanced Multi-Asset Analyzer 2026 - FULLY FEATURED")
        st.markdown("**Complete Trading Suite: Analysis, Watchlist, Portfolio, Alerts, Backtesting & More**")
        ...
        if kill_zone['active']:
            ...
            <div class="success-box">
                <h3 style="margin:0; color:white !important;">🎯 {kill_zone['name']} - ACTIVE</h3>
                <p style="margin:5px 0 0 0;">{kill_zone['description']} | Multiplier: {kill_zone['multiplier']}x | Priority: {kill_zone['priority']}/5</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="info-box">
                <h3 style="margin:0;">{kill_zone['name']}</h3>
                <p style="margin:5px 0 0 0;">{kill_zone['description']}</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.button("🚀 Start Analysis", use_container_width=True):
                ...
                if "Stocks" in asset_type:
                    ...
                    if results:
                        ...
                        st.subheader(f"💹 {top_pick['name']} ({top_pick['symbol']}) - Advanced Chart")
        ...
        # PAGE: SETTINGS
            ...
            with col1:
                theme = st.selectbox("Theme", ["Dark (TradingView)", "Light", "Custom"],
                                     index=0 if st.session_state.preferences['theme'] == 'dark' else 1)
                st.session_state.preferences['theme'] = 'dark' if theme == "Dark (TradingView)" else 'light'
        ...
        st.sidebar.markdown("---")
        st.sidebar.success("🚀 **ICT Analyzer Pro 2026** - All Features Included!")
