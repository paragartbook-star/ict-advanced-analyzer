...
def main():
    ...
    st.markdown(f"<div class='small-text' style='text-align: right;'>🕒 {current_time} IST</div>", unsafe_allow_html=True)
...
def render_market_analysis():
    ...
    analysis_type = st.selectbox("Select Analysis Type:", ["Stock Analysis", "Crypto Analysis", "Technical Analysis", "Pattern Detection"])
...
def render_stock_analysis():
    selected_stocks = st.multiselect("Select Stocks:", NIFTY_50[:10], default=['RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS'])
...
def render_crypto_analysis():
    selected_crypto = st.multiselect("Select Cryptocurrencies:", TOP_CRYPTO, default=['BTC-USD', 'ETH-USD', 'BNB-USD'])
...
def display_analysis_results(analysis):
    ...
    with col3:
        ...
        st.markdown(f"<span class='{analysis['signal_color']}' style='font-size: 16px;'>{analysis['signal']}</span>", unsafe_allow_html=True)
